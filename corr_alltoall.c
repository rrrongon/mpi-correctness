 ...
 * 	*/
	...
	* */
		...
		float * input_int = malloc(sizeof(float) * num_elements_per_proc * size);
		int index=0;
		int i;
		for (i=0; i< size; i++){
			int j;
			for (j=0; j< num_elements_per_proc; j++){
				input_int[index] = i * size * 10 + j*10 ;
				index++;
			}
		}
	...
	* */
		float * my_input = malloc(sizeof(float) * size);
		for(i = 0; i < size; i++){
			int j;
			for(j=0; j< num_elements_per_proc; j++){
				
			}
			my_input[i] = my_rank * size * 10 + i * 10;
		}
	...
	* */
		float* buffer_recv = malloc(sizeof(float) * size);
