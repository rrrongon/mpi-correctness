...
int main(int argc, char* argv[]){
		...
		for (size_counter=0; size_counter<SIZES; size_counter++){
				...
				if(my_rank==0){
						...
						*/
						...
						for(rank=1;rank< size; rank++){
								rank_pass = true;
	    						MPI_Recv(min_val_recv, data_count, MPI_FLOAT, i,1, MPI_COMM_WORLD, &status);
	    						int j;
			...
			//Create New world
					...
					int data_count = num_elements_per_proc;
					MPI_Bcast(rand_nums, data_count, MPI_FLOAT, root, New_Comm);
					...
					if(new_id==0){
							...
							for(rank=1;rank< new_world_size; rank++){
									rank_pass = true;
									MPI_Recv(subcomm_recv_val, data_count, MPI_FLOAT, i,1, New_Comm, &status);
							...
							int destination_rank=0;
							MPI_Send(rand_nums, num_elements_per_proc, MPI_FLOAT, destination_rank, 1, MPI_COMM_WORLD);	
					}
