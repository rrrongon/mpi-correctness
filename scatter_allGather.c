...
int main(int argc, char** argv){
    ...
    MPI_Allgather(&avg_val, 1, MPI_FLOAT, sub_avgs, 1, MPI_FLOAT, MPI_COMM_WORLD);
    
    float final_avg = compute_average(sub_avgs, world_size);
        
    printf("Avg of all elements is %f from processor %s-rank %d\n", processor_name, world_rank,final_avg);
